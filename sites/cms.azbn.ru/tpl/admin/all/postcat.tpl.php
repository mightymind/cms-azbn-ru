...
function showEntityWithChildren(&$catalog,$entity_id,$entity_type='postcat') {
	?>
	<div class="row" >
...
<div class="page-header" >
	<h3>
		Список разделов постов
		<a href="/admin/add/postcat/" title="Создать категорию" ><img class="icon" src="/img/cms.azbn.ru/add.png" /></a>
	</h3>
</div>
	...
	if(count($catalog['structure'][0])) {
		foreach($catalog['structure'][0] as $index=>$entity) {
			showEntityWithChildren($catalog,$index,'postcat');
			}
		}
	?>