...
function showEntityWithChildren(&$catalog,$entity_id,$entity_type='geopointcat') {
	?>
	<div class="row" >
...
<div class="page-header" >
	<h3>
		Список категорий геоотметок
		<a href="/admin/add/geopointcat/" title="Создать категорию" ><img class="icon" src="/img/cms.azbn.ru/add.png" /></a>
	</h3>
</div>
	...
	if(count($catalog['structure'][0])) {
		foreach($catalog['structure'][0] as $index=>$entity) {
			showEntityWithChildren($catalog,$index,'geopointcat');
			}
		}
	?>