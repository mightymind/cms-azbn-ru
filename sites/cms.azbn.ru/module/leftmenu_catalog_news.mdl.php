...
function showEntityWithChildren(&$catalog,$entity_id) {
	echo '<div class="leftmenu-catalog-item" >
		<a href="/news/cat/'.$catalog['list'][$entity_id]['id'].'/" >'.$catalog['list'][$entity_id]['title'].'</a>';
...
$item_list=$this->FE->DB->dbSelect("SELECT * FROM `".$this->FE->DB->dbtables['t_newscat']."` ORDER BY id");
